...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("DrugLifecycleContractService") with @Autowired to get this Bean
   */
  @Bean("DrugLifecycleContractService")
  public Map<String, DrugLifecycleContractService> initDrugLifecycleContractServiceManager() throws Exception {
    Map<String, DrugLifecycleContractService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	String privateKey = this.hexPrivateKeyList.get(i);
    	if (privateKey.startsWith("0x") || privateKey.startsWith("0X")) {
    		privateKey = privateKey.substring(2);
    	...
    	String userAddress = cryptoKeyPair.getAddress();
    	log.info("++++++++hexPrivateKeyList[{}]:{},userAddress:{}", i, privateKey, userAddress);
    	DrugLifecycleContractService drugLifecycleContractService = new DrugLifecycleContractService();
    	drugLifecycleContractService.setAddress(this.config.getContract().getDrugLifecycleContractAddress());
    	drugLifecycleContractService.setClient(this.client);
    	org.fisco.bcos.sdk.transaction.manager.AssembleTransactionProcessor txProcessor = 
    		org.fisco.bcos.sdk.transaction.manager.TransactionProcessorFactory.createAssembleTransactionProcessor(this.client, cryptoKeyPair);
    	drugLifecycleContractService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, drugLifecycleContractService);
    }
    log.info("++++++++DrugLifecycleContractService map:{}", serviceMap);
    return serviceMap;
  }
}
